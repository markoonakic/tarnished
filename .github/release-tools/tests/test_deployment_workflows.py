...
class DeploymentWorkflowTests(unittest.TestCase):
    def test_ci_workflow_validates_deployment_surfaces(self) -> None:
        ...
        self.assertIn("docker compose -f docker-compose.yml config", content)
        self.assertIn("docker compose -f docker-compose.postgres.yml config", content)
        self.assertIn("docker build -t tarnished:ci .", content)
        self.assertIn("helm lint chart", content)
        self.assertIn("helm template tarnished ./chart", content)

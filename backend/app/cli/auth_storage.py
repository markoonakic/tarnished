...
from app.cli.config import resolve_config_dir
...
def load_auth(
    ...
    *,
    ...
    if prefer_keyring:
        try:
            payload = keyring.get_password(KEYRING_SERVICE, profile)
        ...
        try:
            if auth.is_empty():
                ...
                keyring.set_password(
                    KEYRING_SERVICE, profile, auth.model_dump_json(exclude_none=False)
                )
